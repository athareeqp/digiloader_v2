...
const Cards = () => {
  ...
  useEffect(() => {
    const doSearch = (e) => {
      axios.get("https://digimoncard.io/api-public/search.php?n=" + name).then((results) => {
    ...
    return (
      <main>
        <div className="search">
          <p>
            Search: <input type="text" placeholder="Search by name" value={name} onChange={(e) => setName(e.target.value)} />
          </p>
        </div>
  ...
  return (
    <div className="random-page d-flex flex-column align-items-center">
      ...
      {isLoading ? <div className="text-white text-center mb-3">Loading...</div> : <div className="container gifs mb-3">{renderList()}</div>}
    </div>
  );
};
